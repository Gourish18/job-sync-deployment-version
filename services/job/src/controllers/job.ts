...
//       throw new ErrorHandler(400,"Failed to create file buffer");
//     }
//     const {data}=await axios.post(`${process.env.UPLOAD_SERVICE}/api/utils/upload`,{
//       buffer:fileBuffer.content,
//     });
//     const [newCompany] =
//   await sql`INSERT INTO companies (name, description, website, logo,
//   logo_public_id, recruiter_id)
...
export const createCompany = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    if (!data || !data.url || !data.public_id) {
      throw new ErrorHandler(500, "Logo upload failed");
    }
...
export const createJob = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  const [company] = await sql`SELECT company_id FROM companies 
            WHERE company_id = ${company_id} 
            AND recruiter_id = ${user.user_id}`;
  if (!company) {
    throw new ErrorHandler(
      404,
      "Company not found or you're not authorized to add jobs to it.",
    );
  }
  const [newJob] =
    await sql`INSERT INTO jobs (title, description, salary, location, role,
...
`;
  ...
  if (existingJob.posted_by_recruiter_id !== user.user_id) {
    throw new ErrorHandler(403, "You are not authorized to update this job");
  }
...
export const getAllActiveJobs = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    let querySting = `
      SELECT j.job_id, j.title, j.description, j.salary, j.location, 
...
export const getAllApplicationForJob = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    if (user.role !== "recruiter") {
      throw new ErrorHandler(
        403,
        "Forbidden: Only recruiter can access the applications for a job",
      );
    }
...
export const updateApplication = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    // ✅ authorization check
    if (job.posted_by_recruiter_id !== user.user_id) {
      throw new ErrorHandler(403, "Forbidden: Not your job");
    }
